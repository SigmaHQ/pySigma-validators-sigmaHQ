...
def _load_sigmahq_json() -> Dict[str, str]:
    """Load JSON data from the SigmaHQ provider source (URL or local file) and cache it.

    Returns:
        dict: A dictionary containing the version and category_no_provider data with keys:
            - 'sigmahq_category_no_provider_version': The version string
            - 'sigmahq_category_no_provider': The event ID mapping data (both strings)
    """
    cache = _get_cache()
...
def _get_cached_data() -> Dict[str, str]:
    ...
    Returns:
        dict: The cached data containing version and category_no_provider strings.
    """
